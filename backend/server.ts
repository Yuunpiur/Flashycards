    ...
    db.query(sql, userName, (err, data) => {
        ...
        if (data === undefined || data.length === 0) {
            ...
            bcrypt.compare(userPassword, data[0]["password"], (err, result) => {
                if (err) { console.log("Error in comparing: ", err); return; }
                if (result) {
    ...
    db.query(sql, [username], (err, data) => {
        ...
        if (data.length > 0) {
            ...
            bcrypt.genSalt(saltRounds, (err, salt) => {
                if (err) { console.log("Error on the genSalt func: ", err); return; }

                bcrypt.hash(req.body.userPassword, salt, (err, hash) => {
                    if (err) { console.log("Error on the hash func: ", err); return; }
